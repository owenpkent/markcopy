...
- **Right-click a link or an email address to copy it.** Both were reachable only by dragging across the text and hoping the selection started and ended in the right place, which is exactly the kind of value a hand-drag gets wrong invisibly. The menu now leads with **Copy Email Address** or **Copy Link** whenever the pointer is on one, ahead of even a selection, since right-clicking a link names a single element where a selection names a range.
