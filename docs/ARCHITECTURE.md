...
| `src/csv.ts`                     | CSV/TSV support on the host: an RFC 4180 parser (quoted fields, embedded delimiters/newlines, BOM, CRLF/LF/CR, per-record source lines, a row cap that still counts what it skipped), delimiter sniffing, and the grid HTML. Pure, unit-tested.                                                                                                                        |
...
| `src/webview/table.ts`           | CSV/TSV table serialization for the clipboard (RFC 4180). Skips cells marked `data-mc-ignore`, which is how the CSV grid's row-number gutter stays out of copies. Pure, unit-tested.                                                                                                                                                                                   |
...
| `editCell`      | `line`, `column`, `value`, `docVersion` | One edited CSV cell. The host rewrites that field in the document (see [CSV editing](#csv-editing)); `docVersion` is dropped if it no longer matches, so a stale grid cannot edit the wrong row.                                                                                                                                                        |
...
- **The render message carries `docVersion`**, which the grid echoes back with the edit. If the document has moved on since that render, the edit is dropped rather than applied against stale line numbers; the re-render already in flight carries the truth.
