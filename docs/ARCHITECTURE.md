...
Dark mode for PDF pages is applied by inverting the rendered canvas bitmap at render time, not a CSS filter or a pdf.js rendering mode: after `page.render()` finishes, `pdf.ts` sets `globalCompositeOperation = 'difference'`, fills the canvas with white, and restores it, which is an exact colour inversion (no hue rotation) of the actual pixels. An earlier version used a CSS filter (`filter: invert(1) hue-rotate(180deg)` on the canvas element, the same trick used by [Folio](https://github.com/owenpkent/folio)), but a CSS filter forces the browser to re-rasterise the layer at CSS resolution, which blurs pages on HiDPI displays; inverting the bitmap keeps the oversampled render crisp. It follows `markcopy.theme` (auto/light/dark) like the Markdown preview, with a right-click **Dark Pages** / **Light Pages** item that overrides it for the session (`pageMode` in `pdf.ts`, one of `auto` / `normal` / `inverted`); toggling it clears every rendered page and re-rasterises the visible ones, since the inversion is baked into the bitmap rather than a live filter. **Copy Page as PNG** still copies the true, non-inverted page even while dark mode is on, by inverting a throwaway copy of the canvas back before encoding it. A `forced-color-adjust: none` rule on the page root keeps Windows High Contrast from recoloring page content.
