...
2. Create an Azure DevOps organization, signed in as the **same account that owns the publisher**. This is already done for this publisher (the `owenpkent` org), so it is a first-time-only step. If you ever need it again: <https://aex.dev.azure.com> → **Create new organization** (accept the defaults; the org name and any project are irrelevant). `aex.dev.azure.com` is the reliable entry point only while no org exists yet, because plain <https://dev.azure.com> redirects a fresh account to the marketing page.
3. Create the PAT at <https://dev.azure.com/owenpkent/_usersSettings/tokens> → **New Token**. (That page works now that the org exists; you can also reach it from **User Settings**, the gear icon top-right, _not_ the avatar, → **Personal access tokens**.) Two settings are load-bearing:
   - **Organization = All accessible organizations.** An org-scoped token fails with a 401 even when the scopes are right.
   - **Scope: Marketplace → Manage**, behind **Show all scopes**.

   Set the expiration to the maximum (1 year) while you are there. The default is 30 days, and a silently expired token is what broke publishing just before the 0.8.0 release. Copy the token; it is shown once.
